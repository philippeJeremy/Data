...
world_cases["moving_average"] = world_cases["cases"].rolling(window=7, min_periods=1).mean()
...
previous_growth_rate = cases_per_country["moving_average"].iloc[-2] / cases_per_country["moving_average"].iloc[-3]
...
with content:
    st.metric("Current growth rate", np.round(current_growth_rate, 3), np.round(growth_rate_evolution, 3), "inverse")
    st.caption("Growth rate compares the average number of cases of today to the average number of cases of yesterday. If growth rate > 1 - the virus is spreading. If growth rate < 1 - the wave is slowing down")
#####
